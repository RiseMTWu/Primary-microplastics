# -*- coding: utf-8 -*-
"""
Created on Tue Jul  1 10:31:30 2025
...
# #Vehicle ownership
# data_input = pd.read_csv(r"../Vehicle ownership/dataset/TWP_dataset_decrease_10.csv",encoding="GBK")
#Road mileage
# data_input = pd.read_csv(r"../Road mileage/dataset/TWP_dataset_increase_10.csv",encoding="GBK")
#Road mileage ratio
#data_input = pd.read_csv(r"../Road mileage ratio/dataset/TWP_dataset_vary_30.csv",encoding="GBK")
#Sewage treatment facilities
data_input = pd.read_csv(r"../Sewage treatment facilities/dataset/Remove_1.1.csv",encoding="GBK")
...
action_columns = ['small1', 'cityroadA', 'M1', 'ruralroadA', 'M2', 'highwayA', 'M3',
                  ...
                  'Car-percent', 'Truck-percent', 'Airplaneinput','TCR',
...
for i in range(3060):
    ...
    action['Car-percent'] = np.random.normal(loc=data_input['Car-percent'][i], scale=score_mapping['Car-percent'] * data_input['Car-percent'][i], size=10000)
    action['Car-percent'][action['Car-percent'] < 0] = 0.0001
    action['Truck-percent'] = np.random.normal(loc=data_input['Truck-percent'][i], scale=score_mapping['Truck-percent']  * data_input['Truck-percent'][i], size=10000)
    action['Truck-percent'][action['Truck-percent'] < 0] = 0.0001
    ...
    micro['BOX1'] = ((action['small1'] * (action['cityroadA'] * action['M1'] + action['ruralroadA'] * action['M2'] + action['highwayA'] * action['M3'])
                ...
                + action['big1'] * (action['cityroadC'] * action['M7'] + action['ruralroadC'] * action['M8'] + action['highwayC'] * action['M9'])) * action['TCR'] * action['Car-percent'] * 0.00001 
               + (action['small2'] * (action['cityroadD'] * action['M10'] + action['ruralroadD'] * action['M11'] + action['highwayD'] * action['M12'])
                ...
                + action['big2'] * (action['cityroadF'] * action['M16'] + action['ruralroadF'] * action['M17'] + action['highwayF'] * action['M18']))* action['TCR'] * action['Truck-percent'] * 0.00001 ) 
...
city.to_csv("../Remove_1.1_results.csv", index=True)
#%%
###############################           EF sensitivity
import pandas as pd
import warnings
...
data = pd.read_csv(r"../2019_TWP_dataset.csv",encoding="GBK")
print(data.shape) 
para_df = pd.read_csv(r"../TWP_para_score.csv",encoding="GBK")
score_mapping = dict(zip(para_df['Parameters'], para_df['Score']))

columns_to_adjust = ['M1', 'M2', 'M3', 'M4', 'M5', 'M6', 'M7', 'M8', 'M9', 
                     'M10', 'M11', 'M12', 'M13', 'M14', 'M15', 'M16', 'M17', 'M18']


adjustments = [1.10, 1.20, 1.30, 0.90, 0.80, 0.70]  # +10%, +20%, +30%, -10%, -20%, -30%
...
for adjustment in adjustments:
    ...
    for col in columns_to_adjust:
        data_input[col] = data[col] * adjustment
    ...
    city.columns = data_input['City']
    city.to_csv("../TWP_vary_{}_results.csv".format(adjustment), index=True)
