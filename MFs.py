# -*- coding: utf-8 -*-
"""
Created on Mon Aug 19 10:04:34 2024
...
action = pd.DataFrame(index=range(10000), columns=['2019年人口', '合成纤维比例', '机洗频率', '手洗频率', '机洗排放因子',
                                                   '手洗排放因子', '机洗负荷量', '手洗负荷量', 
                                                   'TC14', 'TC15', 'TC145',
...
for i in range(3060):
    ...
    action['2019年人口'] = np.random.normal(loc=data_input['2019年人口'][i], scale=0.42 * data_input['2019年人口'][i], size=10000)
    action['合成纤维比例'] = np.random.normal(loc=data_input['合成纤维比例'][i], scale=0.15*data_input['合成纤维比例'][i], size=10000)
    action['机洗频率'] = np.random.normal(loc=data_input['机洗频率'][i], scale=0.43 * data_input['机洗频率'][i], size=10000)
    action['手洗频率'] = np.random.normal(loc=data_input['手洗频率'][i], scale=0.43 * data_input['手洗频率'][i], size=10000)
    action['机洗排放因子'] = np.random.normal(loc=data_input['机洗排放因子'][i], scale=0.17 * data_input['机洗排放因子'][i], size=10000)
    action['手洗排放因子'] = np.random.normal(loc=data_input['手洗排放因子'][i], scale=0.17 * data_input['手洗排放因子'][i], size=10000)
    action['机洗负荷量'] = np.random.normal(loc=data_input['机洗负荷量'][i], scale=0.42 * data_input['机洗负荷量'][i], size=10000)
    action['手洗负荷量'] = np.random.normal(loc=data_input['手洗负荷量'][i], scale=0.42 * data_input['手洗负荷量'][i], size=10000)
    
    action['2019年人口'][action['2019年人口'] < 0] = 0.0001
    action['合成纤维比例'][action['合成纤维比例'] < 0] = 0.0001
    action['机洗频率'][action['机洗频率'] < 0] = 0.0001
    action['手洗频率'][action['手洗频率'] < 0] = 0.0001
    action['机洗排放因子'][action['机洗排放因子'] < 0] = 0.0001
    action['手洗排放因子'][action['手洗排放因子'] < 0] = 0.0001
    action['机洗负荷量'][action['机洗负荷量'] < 0] = 0.0001
    action['手洗负荷量'][action['手洗负荷量'] < 0] = 0.0001
    ...
    action['TC15'] = np.random.normal(loc=data_input['TC15'][i], scale=0.43 * data_input['TC15'][i], size=10000)
    ...
    micro['BOX1'] = action['2019年人口'] * action['合成纤维比例'] * action['机洗频率']* action['机洗负荷量']* action['机洗排放因子']*0.00001
    #手洗
    micro['BOX2'] = action['2019年人口'] * action['合成纤维比例'] * action['手洗频率']* action['手洗负荷量']* action['手洗排放因子']*0.00001
    #机洗+手洗
    ...
    a2.extend(np.quantile(micro['BOX1'], [0.05, 0.25, 0.75, 0.95]))  #机洗
    
    a3 = [np.mean(micro['BOX2']), np.std(micro['BOX2'])]
    a3.extend(np.quantile(micro['BOX2'], [0.05, 0.25, 0.75, 0.95])) #手洗
    
    a4 = [np.mean(micro['BOX16']), np.std(micro['BOX16'])]
    a4.extend(np.quantile(micro['BOX16'], [0.05, 0.25, 0.75, 0.95])) #海洋污染
    
    a5 = [np.mean(micro['BOX17']), np.std(micro['BOX17'])]
    a5.extend(np.quantile(micro['BOX17'], [0.05, 0.25, 0.75, 0.95])) #残余土壤污染
    
    a6 = [np.mean(micro['BOX18']), np.std(micro['BOX18'])]
    a6.extend(np.quantile(micro['BOX18'], [0.05, 0.25, 0.75, 0.95])) #残余河流污染
...
city.to_csv("MFs—result.csv", index=True)    
